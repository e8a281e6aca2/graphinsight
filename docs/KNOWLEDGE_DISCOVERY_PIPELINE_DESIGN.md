...
当前状态：部分完成。实体归一化已清理 LLM 返回的 JSON/dict 污染，并对常见药剂名称空格差异做规范化；表格 chunk 已按“首列主体 -> 其他列指标”生成行级关系，关系写入 `evidence/relation_type/confidence`。LLM 关系抽取已改为 schema-aware 输出，候选关系类型收敛为防治对象、平均防效、病情指数、产量、增产率、使用剂量、发生阶段、地点、属于、影响等，并要求输出 `source/target/label/evidence/confidence`。旧规则抽取已禁止把任意短语当作关系类型，低置信度“同段提及”不进入默认关系集合；当前 PDF 复建结果为 `Chunk=14`、`Entity=177`、`RELATION=77`，无 JSON/dict 脏实体，全部 RELATION 均带 evidence。仍需接入 DocumentProfiler 和按文档类型 prompt。
...
1. 实现 `StructuredChunker`，替换固定字符窗口为结构化 chunk。
2. 修复实体抽取 JSON 污染，先保证实体干净。
3. 增加 `DocumentProfiler` 的最小实现，输出文档类型与推荐 schema。
4. 将当前 PDF 作为验收样例，同时补合同/财报/手册各一个小样例。
5. 再做 schema-aware entity/relation extraction。
