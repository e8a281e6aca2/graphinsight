...
health_check() {
  powershell.exe -NoProfile -Command "try { \$resp = Invoke-RestMethod -Method Get -Uri '$BACKEND_HEALTH_URL/health' -TimeoutSec 5; if (\$resp.code -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }" >/dev/null 2>&1
}
...
if [[ -z "$ADMIN_PASSWORD" && -z "$ADMIN_TOKEN" ]]; then
  echo "Missing ADMIN_PASSWORD or ADMIN_TOKEN for frontend E2E."
  exit 1
fi
...
if ! health_check; then
  echo "BACKEND_HEALTH_FAIL url=$BACKEND_HEALTH_URL"
