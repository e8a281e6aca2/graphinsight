#!/usr/bin/env python3
"""Verify NL2Cypher POST writes Go-side admin audit logs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check NL2Cypher Go audit contract")
    ...
    _assert(bool(token), "ADMIN_TOKEN is required for NL2Cypher audit contract check")
    base_url = args.base_url.rstrip("/")
    trace_id = f"nl2cypher-audit-{uuid.uuid4().hex}"

    status, body, _ = _request(
        "POST",
        f"{base_url}/api/nl2cypher",
        token=token,
        trace_id=trace_id,
        payload={"natural_language": "   "},
    )
    _assert(status == 400, f"expected 400 for blank natural_language, got status={status} body={body}")
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        if item.get("action") == "nl2cypher_generate" and item.get("trace_id") == trace_id:
            found = item
            break
    _assert(found is not None, f"expected nl2cypher_generate audit log for trace_id={trace_id}, got items={items}")
    ...
    print("NL2CYPHER_AUDIT_GO_CONTRACT_OK")
    return 0
