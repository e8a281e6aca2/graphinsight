#!/usr/bin/env python3
"""Deep research POST Go entry contract smoke check."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check deep research POST Go entry contract")
    ...
    _assert(bool(token), "ADMIN_TOKEN is required for /api/docqa/deep-research POST contract check")

    status, body, headers = _request(f"{base_url}/api/docqa/deep-research", {"question": "   "}, token=token)
    ...
    print("DEEP_RESEARCH_POST_GO_CONTRACT_OK")
    return 0
