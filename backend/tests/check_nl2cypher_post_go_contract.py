#!/usr/bin/env python3
"""NL2Cypher POST Go entry contract smoke check."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check NL2Cypher POST Go entry contract")
    ...
    _assert(bool(token), "ADMIN_TOKEN is required for /api/nl2cypher POST contract check")

    status, body, headers = _request(f"{base_url}/api/nl2cypher", {"natural_language": "   "}, token=token)
    ...
    _assert(body.get("message") == "自然语言查询不能为空", f"unexpected message body={body}")
    _assert(body.get("trace_id"), f"expected trace_id in body, got body={body}")

    print("NL2CYPHER_POST_GO_CONTRACT_OK")
    return 0
