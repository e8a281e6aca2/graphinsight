#!/usr/bin/env python3
"""Verify admin_users.login_count migration supports migrate, dry-run, and rollback."""
...
SCRIPT = REPO_ROOT / "backend" / "admin" / "migrate_add_login_count.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="graphinsight-migrate-login-count-") as tmpdir:
        ...
        _assert("login_count" in _columns(db_path), f"login_count missing after migrate: {_columns(db_path)}")
    ...
    print("MIGRATE_ADD_LOGIN_COUNT_ROLLBACK_UNIT_OK")
    return 0
