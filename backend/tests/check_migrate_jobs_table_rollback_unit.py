#!/usr/bin/env python3
"""Verify admin_jobs table migration supports migrate, dry-run, and rollback."""
...
SCRIPT = REPO_ROOT / "backend" / "admin" / "migrate_jobs_table.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="graphinsight-migrate-jobs-table-") as tmpdir:
        ...
        _assert("admin_jobs" not in baseline_tables, f"admin_jobs should not exist before migrate: {baseline_tables}")
        ...
        _assert(_tables(db_path) == baseline_tables, "dry-run should not create admin_jobs")

        migrate = _run(db_url, env_file)
        _assert(migrate.returncode == 0, f"migrate failed: {migrate.stdout}\n{migrate.stderr}")
        migrated_tables = _tables(db_path)
        _assert("admin_jobs" in migrated_tables, f"admin_jobs missing after migrate: {migrated_tables}")
        ...
        _assert(_tables(db_path) == migrated_tables, "rollback dry-run should not drop admin_jobs")

        rollback = _run(db_url, env_file, "--action", "rollback")
        _assert(rollback.returncode == 0, f"rollback failed: {rollback.stdout}\n{rollback.stderr}")
        _assert("admin_jobs" not in _tables(db_path), "admin_jobs should be removed after rollback")
    ...
    print("MIGRATE_JOBS_TABLE_ROLLBACK_UNIT_OK")
    return 0
