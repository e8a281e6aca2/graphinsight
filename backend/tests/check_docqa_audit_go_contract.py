#!/usr/bin/env python3
"""Verify DocQA POST writes Go-side admin audit logs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check DocQA Go audit contract")
    ...
    _assert(bool(token), "ADMIN_TOKEN is required for DocQA audit contract check")
    base_url = args.base_url.rstrip("/")
    trace_id = f"docqa-audit-{uuid.uuid4().hex}"
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        if item.get("action") == "docqa_ask" and item.get("trace_id") == trace_id:
            found = item
            break
    _assert(found is not None, f"expected docqa_ask audit log for trace_id={trace_id}, got items={items}")
    ...
    print("DOCQA_AUDIT_GO_CONTRACT_OK")
    return 0
